...
import { DeleteCamelAsync, ViewCamelAsync } from '../services/actions/SubmitAction';
  ...
  return (
    <Modal show={isDelete}>
      <Modal.Body className='text-center p-4 border border-secondary rounded-4 bg-dark'>
        ...
        <Button variant="primary" className='me-3 px-4' onClick={() => disptch(ViewCamelAsync())}><i className="bi bi-x-circle"></i>
        </Button>
        <Button variant="danger" className='px-4' onClick={() => disptch(DeleteCamelAsync(deleteId))}><i className="bi bi-trash-fill"></i></Button>
      </Modal.Body>
    </Modal>
  );
}
